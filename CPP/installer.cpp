...
static StepResult Step_OAuth(const fs::path& projectRoot)
{
    ...
    std::cout << "  A browser window will open for Google authorization.\n"
              ...
              << "       Forms, Classroom, Meet, Drive Labels, and Gmail\n"
              << "     - Click 'Continue'\n"
...
static void PrintDone()
{
    ...
    std::cout << "  1. Restart Claude Desktop\n"
              ...
              << "  You now have 330 tools across 10 Google APIs:\n"
              << "  Drive, Calendar, Docs, Sheets, Slides,\n"
              << "  Forms, Classroom, Meet, Drive Labels, and Gmail.\n\n";
}

// ─── Download Step (Git or ZIP fallback) ─────────────────────────────────────

static bool HasGit()
{
    return !FindInPath("git").empty();
}

static StepResult Step_Download(const fs::path& installDir)
{
    ...
    if (useGit) {
        ...
        std::cout << "\n  Downloading Google MCP (ZIP) to " << installDir.string() << "...\n";

        // Download ZIP via PowerShell
        std::string zipPath = (fs::temp_directory_path() / "google-mcp.zip").string();
        std::string dlCmd =
            "powershell -NoProfile -Command \""
            "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; "
            "Invoke-WebRequest -Uri 'https://github.com/leeisfailing/google-mcp/archive/refs/heads/main.zip' "
            "-OutFile '" + zipPath + "'\"";
        int rc = RunCommandInteractive(dlCmd);
        if (rc != 0 || !fs::exists(zipPath))
            return { false, "Download failed (exit code " + std::to_string(rc) + ")\n\n"
                            "  Check your internet connection and try again." };
        ...
        std::string extractCmd =
            ...
            "' -DestinationPath '" + fs::temp_directory_path().string() + "' -Force\"";
        RunCommandInteractive(extractCmd);

        // GitHub ZIP extracts to google-mcp-main/ — move contents to installDir
        fs::path extractedDir = fs::temp_directory_path() / "google-mcp-main";
        if (!fs::exists(extractedDir))
            return { false, "Extraction failed — google-mcp-main folder not found in ZIP." };

        std::error_code ec;
        fs::rename(extractedDir, installDir, ec);
        if (ec) {
            // rename across volumes fails — copy instead
            fs::copy_directories(extractedDir, installDir, ec);
    ...
    if (!fs::exists(installDir / "package.json"))
        return { false, "Download succeeded but package.json not found." };

    return { true, useGit ? "Cloned to " + installDir.string()
                          : "Downloaded and extracted to " + installDir.string() };
}
...
int main(int argc, char* argv[])
{
    ...
    // Step 3: Download (Git clone or ZIP fallback)
    PrintStep(3, "Downloading Google MCP...");
    r = Step_Download(projectRoot);
    if (!r.ok) { PrintFail(r.msg); std::cin.get(); return 1; }
    PrintOk(r.msg);

    // Step 4: npm install
    PrintStep(4, "Installing dependencies (npm install)...");
    r = Step_NpmInstall(projectRoot);
    if (!r.ok) { PrintFail(r.msg); std::cin.get(); return 1; }
    PrintOk(r.msg);

    // Step 5: npm run build
    PrintStep(5, "Building MCP server (npm run build)...");
    r = Step_NpmBuild(projectRoot);
    if (!r.ok) { PrintFail(r.msg); std::cin.get(); return 1; }
    PrintOk(r.msg);

    // Step 6: Handle key.json — open file dialog if not found
    PrintStep(6, "Checking key.json...");

    // First check if it's already in the install dir
    fs::path destKey = projectRoot / "key.json";
    bool keyFound = fs::exists(destKey) && Step_CheckKeyJson(projectRoot).ok;

    // Also accept via drag-and-drop (argv[1])
    if (!keyFound && argc >= 2) {
        fs::path keyPath = argv[1];
        std::string s = keyPath.string();
        if (s.size() >= 2 && s.front() == '"' && s.back() == '"')
            keyPath = s.substr(1, s.size() - 2);
        if (fs::exists(keyPath)) {
            std::copy(std::istreambuf_iterator<char>(std::ifstream(keyPath, std::ios::binary)),
                      std::istreambuf_iterator<char>(),
                      std::ostreambuf_iterator<char>(
                          std::ofstream(destKey, std::ios::binary)));
            keyFound = Step_CheckKeyJson(projectRoot).ok;
        }
    }

    // If still not found, open Windows file dialog
    if (!keyFound) {
        Red();
        std::cout << "\n  key.json not found!\n";
        Reset();
        std::cout << "  A file picker will open — select your key.json file.\n"
                  << "  (It's usually named 'client_secret_*.json' — rename it to key.json)\n\n";

        // Build instructions for first-time users
        std::cout << "\n"
            "  If you don't have key.json yet:\n"
            "  1. Go to https://console.cloud.google.com/\n"
            "  2. Create a new project (or select one)\n"
            "  3. Enable ALL of these APIs:\n"
            "     - Google Drive API\n"
            "     - Google Calendar API\n"
            "     - Google Docs API\n"
            "     - Google Sheets API\n"
        ...
        ofn.lpstrTitle = "Select key.json (Google API credentials)";
        ofn.Flags = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;

        if (GetOpenFileNameA(&ofn) && strlen(selectedPath) > 0) {
            // Copy selected file to install dir as key.json
            std::error_code ec;
            fs::copy_file(selectedPath, destKey, fs::copy_options::overwrite_existing, ec);
            if (ec) {
                PrintFail("Failed to copy key.json: " + ec.message() + "\n");
                std::cin.get();
                return 1;
            }
            keyFound = Step_CheckKeyJson(projectRoot).ok;
            if (!keyFound) {
                PrintFail("Selected file is not a valid Google API credentials JSON.\n");
                std::cin.get();
                return 1;
            }
            PrintOk("key.json copied to C:\\google-mcp");
        } else {
            PrintFail("No file selected. Run the installer again with key.json ready.\n");
            std::cin.get();
            return 1;
        }
    } else {
        PrintOk("key.json found");
    }

    // Step 7: OAuth
    PrintStep(7, "Google OAuth setup...");
    r = Step_OAuth(projectRoot);
    if (!r.ok) { PrintFail(r.msg); std::cin.get(); return 1; }
    PrintOk(r.msg);

    // Step 8: Install skills
    PrintStep(8, "Installing Claude skills...");
    ...
    PrintStep(9, "Configuring Claude Desktop...");
